...
class AcrobotEnv(core.Env):
    ...
    **REFERENCE:**
    ...
    def __init__(self, length=1.0, mass=1.0, moi=1.0):
        self.viewer = None
    ...
    def _sample_parameter(self):
        ...
        if self.moi_set is not None:
            set_index = self.np_random.randint(len(self.moi_set))
            self.moi = self.np_random.uniform(self.moi_set[set_index][0],
                                              self.moi_set[set_index][1])
    ...
    def step(self, a):
        ...
        info = {}
        if terminal:
            info['success'] = self.steps_in_episode < 80
    ...
    def render(self, mode='human'):
        ...
        if self.viewer is None:
            ...
            bound = self.length + self.length + 0.2  # 2.2 for default
            self.viewer.set_bounds(-bound,bound,-bound,bound)

        if s is None: return None

        p1 = [-self.length *
              cos(s[0]), self.length * sin(s[0])]

        p2 = [p1[0] - self.length * cos(s[0] + s[1]),
              p1[1] + self.length * sin(s[0] + s[1])]

        xys = np.array([[0,0], p1, p2])[:,::-1]
        thetas = [s[0]- pi/2, s[0]+s[1]-pi/2]
        link_lengths = [self.length, self.length]
